...
`CINFO:2 = "Ghostbusters: Frozen Empire"`). Not universal (many report a generic
...
directly against each other. A real run on a Buffy the Vampire Slayer Season 4 disc
concluded two titles were duplicate encodes of "The Initiative" (matching segment
...
- **Zero-result and empty-candidate searches both need a second pass, for different
  ...
  - "Decoy" candidates (empty-metadata entries ranked among real ones by
    ...
    versions of a classic character" problem — Sherlock Holmes is the canonical
