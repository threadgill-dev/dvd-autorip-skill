...
Two titles on a Buffy the Vampire Slayer Season 4 disc had matching segment counts,
near-identical duration, and — independently — both had their dialogue attributed to
"The Initiative" by web search. That combination read as a duplicate encode of the
...
- One disc (`FIDDLER_1__drive0/B1_t00.mkv`, 7.3GB) turned out to be a real,
...
run hit the other case**: MakeMKV failed on `Ocean's Thirteen`'s disc, and then the
